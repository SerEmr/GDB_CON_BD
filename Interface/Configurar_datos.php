    ...
    <div>
        ...
        <div>
            <div class="seccion_titulo_datos">
                ...
                <h1> Actualizar Datos de la Tabla "<?php echo $nombre_tabla?>"</h1>
            ...
            <div class="seccion_tabla_datos">
                ...
                <form action="../php/Update_datos.php" method="post">
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody>   
                            <?php
                                $datoIDsQueryString = $_GET['DatosIDs'] ?? '';
                                $datoIDsArray = explode(',', $datoIDsQueryString);
                                ...
                                if ($stmtColumnas = $connect->prepare($sqlColumnas)) {
                                    ...
                                    while ($fila = $resultadoColumnas->fetch_assoc()) {
                                        $datoIDActual = $datoIDsArray[$contador] ?? null;
                                        $valorActual = '';

                                        if ($datoIDActual) {
                                            $stmtValor = $connect->prepare("SELECT Valor FROM datos WHERE DatoID = ?");
                                            $stmtValor->bind_param("i", $datoIDActual);
                                            $stmtValor->execute();
                                            $resultadoValor = $stmtValor->get_result();
                                            $filaValor = $resultadoValor->fetch_assoc();
                                            $valorActual = $filaValor['Valor'] ?? '';
                                        }

                                        echo "<tr>";
                                        echo "<td>" . htmlspecialchars($fila['Nombre']) . "</td>";
                                        echo "<td>" . htmlspecialchars($fila['Tipo']) . "</td>";
                                        // Envía el DatoID como un campo oculto
                                        echo "<input type='hidden' name='DatoID[" . $fila['ColumnaID'] . "]' value='" . htmlspecialchars($datoIDActual) . "'>";
                                        // Envía el valor que el usuario puede modificar
                                        echo "<td><input type='text' name='Valor[" . $fila['ColumnaID'] . "]' class='form-control' value='" . htmlspecialchars($valorActual) . "'></td>";
                                        echo "</tr>";
